...
- **Ryan's asks after PR 17 (26 September 2026, branch `m3-controls`).**
  - **The upgrade menu.** Ryan picked Select all, the button lit up a second later, and one building upgraded. The gold covered one upgrade (it grows a gold a second) while the button said Upgrade 12. Now the button says "Upgrade 1 of 3" or "Not enough gold" with the gold still needed, upgrades that can be done now come first and the rest fold under Not yet, rows have - and + and All, and the list updates in place instead of being rebuilt when a count changes. The building card has an Upgrade button (the `upgrade` order takes `ids`).
  - **Placing buildings.** Default: a click pins the outline with Build here and Cancel; Enter and Esc work too. Settings keeps one-click placing. Paint (a switch in the building bar and in Settings) places on every free spot a drag passes, for mouse and touch.
  - **Crosshair** in Settings, Accessibility, for mouse, keyboard and touch.
  - **Evidence.** `npm test` 155 of 155, reference 95 of 95, smoke 93 of 93, `npm run ui` 113 of 113 on the test map and on fine Europe, including "Upgrade 1 of 3" with gold for one, the card's Upgrade, Build here, one-click placing, a paint drag placing 5 watchtowers, and the crosshair on desktop and phone.
