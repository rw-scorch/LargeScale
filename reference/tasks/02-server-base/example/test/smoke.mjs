...
const a = await api("/api/register", { name: "ryan", password: "correct horse", invite: INVITE });
const alogin = a.status === 200 ? a : await api("/api/login", { name: "ryan", password: "correct horse" });
check(alogin.status === 200 && alogin.body.account.admin === true, "ryan gets the admin flag from the server");
